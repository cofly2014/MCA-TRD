...
class CROSS_DOMAIN_FSAR(nn.Module):
    ...
    def target_domain_self_supervised(self, target_domain_features_enc, target_domain_features_comb, t_sub_seq_num,
        ...
        negative_sim_all = negative_sim.sum(dim=1) - torch.exp(torch.ones(batch_size).cuda() / temperature)  # 排除自身相似度

        # 计算损失
        positive_sum = positive_sim.sum(dim=1)
        negative_sum = all_sim.sum(dim=1) + negative_sim_all
        loss = -torch.log(positive_sum / negative_sum).mean()
