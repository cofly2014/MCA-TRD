import os
os.environ["CUDA_VISIBLE_DEVICES"] = "3,4,7,0"
import torch
import numpy as np
import argparse
...
from models.base.few_shot_clip import CNN_OTAM_CLIPFSAR,clycleGAN
...
def parse_command_line():
    ...
    # 经常需要修改
    parser.add_argument("--dataset", choices=["ssv2", "kinetics", "hmdb", "ucf"], default="kinetics",
                        help="Dataset to use.")
    ...
    # 经常需要修改
    parser.add_argument("--training_iterations", "-i", type=int, default=75020, help="Number of meta-training iterations.")
    # 经常需要修改
    parser.add_argument("--num_test_tasks", type=int, default=10000, help="number of random tasks to test on.")
    parser.add_argument('--test_iters', nargs='+', type=int, default=[18000,44000,50000],   #100020    #在训练的第几次开始进行测试
    ...
    # 经常需要修改
    parser.add_argument("--test_checkpoint_iter", type=int, default=16000, help="Path to model to load and test.")
    ...
    parser.add_argument("--combination_number_used", type=int, default=18, help="combination_number_used")
    parser.add_argument("--temporal_set", type=int, default=[1,2,3,4,5], help="temporal_set")
    parser.add_argument("--lambdas", type=int, default=[0.5, 1, 0, 0], help="temporal_set")

    parser.add_argument('--prefix', type=int, default=16)
    parser.add_argument('--postfix', type=int, default=16)
...
class Learner:
    ...
    def __init__(self):
        ...
        if self.args.opt == "adam":
            ...
            # for p in self.model.cycleModel.parameters():
            #     p.requires_grad = True
    ...
    def init_model(self):
        model = CNN_OTAM_CLIPFSAR(self.args, self.cfg)
        # model = clycleGAN(self.args, self.cfg)
        model = model.cuda()
   ...
   #task_dict['support_set']-->1 200 3 224 224
    def train_task(self, task_dict):
        ...
        if self.args.single==True:
            model_dict.backward(retain_graph=False)    
            return model_dict,0.5

        class_logits = model_dict['class_logits']
        target_logits = model_dict['dis_logits']
        kl_logits = model_dict['kl_logits']
        gan_logits= model_dict['gan_logits']
        first_logits = model_dict['first_logits']
        second_logits = model_dict['second_logits']
        ...
        second_logits = second_logits.unsqueeze(0)
        ...
        # loss1 = self.loss(first_logits, task_dict["target_labels"].long(), "cuda") / self.args.tasks_per_batch
        loss1 = self.loss(target_logits, task_dict["target_labels"].long(), "cuda") / self.args.tasks_per_batch
        # loss2 = self.loss(second_logits, task_dict["target_labels"].long(), "cuda") / self.args.tasks_per_batch
        # loss2 = kl_logits/self.args.tasks_per_batch

        task_loss_total = lambdas[0] * loss0 + lambdas[1] * loss1 
        # + lambdas[2] * loss2 + lambdas[3]*gan_logits
