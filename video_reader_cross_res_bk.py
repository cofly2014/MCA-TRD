...
class VideoDataset(torch.utils.data.Dataset):
    def __init__(self, args):
        ...
        self.train_split_target = Split()
        self.test_split_target = Split()
    ...
    def get_train_or_test_db_target(self, split=None):
        if split is None:
            get_train_split = self.train
        else:
            # print("split:" + split)
            if split in self.train_test_lists_target["train"]:
                ...
                return None
        if get_train_split:
            return self.train_split_target  # 返回一个容器
        else:
            return self.test_split_target  # 返回一个容器
    ...
    def read_dir_target(self):
        ...
        print("train: {}, test: {}".format(len(self.train_split_target), len(self.test_split_target)))
    ...
    def __getitem__(self, index):
        ...
        #在测试阶段，取target中的test数据
            ...
            c = self.get_train_or_test_db_target() #如果是meta testing阶段 则拿到 目标域的meta-testing阶段的数据
        ...
        if self.train:
            c = self.get_train_or_test_db_target()  #这个方法内部 已经确定了 是选择的是meta-training还是meta-testing
            classes = c.get_unique_classes()  # 其实就是self.gt_a_list 中 不重复元素的列表
            class_numbers = []
            for single_class in classes:
                class_numbers.append(c.get_num_videos_for_class(single_class))
            random_num  = self.args.target_num
            select_video_numbers = random.sample(range(1, len(self.train_split_target)), random_num)
